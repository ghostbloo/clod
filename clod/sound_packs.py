...
class SoundPackManager:
    ...
    def _install_from_directory(self, dir_path: Path, pack_name: str | None = None) -> bool:
        ...
        try:
            ...
            raise ValueError(f"Invalid sound pack schema: {e}")
    ...
    def enable_for_claude_code(self, pack_name: str, matcher: str = "*") -> bool:
        ...
        for event_name, event_config in pack_schema.events.items():
            ...
            dest_path = self.claude_sounds_dir / f"{pack_name}_{event_config.sound_file}"
            
            if source_path.exists():
                shutil.copy2(source_path, dest_path)
                
                # Set up sound mapping
                sfx_manager.set_sound_mapping(
                    claude_event, 
                    matcher, 
                    dest_path.name
                )
    ...
    def _generate_opencode_plugin(
        self, pack_name: str, pack_schema: SoundPackSchema, pack_dir: Path
    ) -> str:
        ...
        plugin_code = f'''// Clod Sound Pack: {pack_schema.name}
// Generated automatically by clod
// Pack version: {pack_schema.version}
// Description: {pack_schema.description}

import {{ spawn }} from "child_process";

const SOUND_MAPPINGS = {json.dumps(event_mappings, indent=2)};

function playSound(soundFile, volume = 1.0) {{
  try {{
    // Use platform-appropriate sound player
    ...
    console.warn(`Failed to play sound ${{soundFile}}:`, error.message);
  }}
}}
...
export default function({{ client, app, $ }}) {{
  return {{
    async event({{ event }}) {{
      ...
      const mapping = SOUND_MAPPINGS[eventType];
      
      if (mapping) {{
        playSound(mapping.sound_file, mapping.volume);
      }}
    }}
  }};
}}
